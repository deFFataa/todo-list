    ...
    <div class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
        <div class="flex justify-between space-x-2">
            ...
            <div class="flex gap-2">
                @if ($todos->completed)
                    <input wire:click="toggle({{ $todos->id }})" type="checkbox" checked>
                @else
                    <input wire:click="toggle({{ $todos->id }})" type="checkbox">
                @endif


                @if ($EditTodoID === $todos->id)
                    <div>
                ...
                @else
                    <h3 class="text-lg text-semibold text-gray-800">{{ $todos->name }}</h3>
                @endif
            </div>
            ...
            <div class="flex items-center space-x-2">
                <button wire:click="edit({{ $todos->id }})"
                ...
                <button wire:click="destroy({{ $todos->id }})"
        ...
        <span class="text-xs text-gray-500"> {{ $todos->created_at }} </span>
